...
try {
    ...
    $actionsRequireProject = ['add_material','remove_material','update_material_status','toggle_step','add_step','update_project_status','publish_shared_project','generate_share_link','update_title','update_description','delete_material','delete_project'];
    ...
    switch ($action) {
        ...
        case 'remove_material':
            $mid = filter_input(INPUT_POST,'material_id',FILTER_VALIDATE_INT); if (!$mid) 
            { echo json_encode(['success'=>false,'message'=>'Invalid material id']); exit; }
            $d = $conn->prepare('DELETE FROM project_materials WHERE material_id = ? AND project_id = ?'); 
            $d->bind_param('ii',$mid,$project_id); $d->execute();
        ...
        case 'toggle_step':
            ...
            $conn->query("CREATE TABLE IF NOT EXISTS project_step_progress (id INT AUTO_INCREMENT PRIMARY KEY, project_id INT NOT NULL, 
            step_id INT NOT NULL, is_done TINYINT(1) DEFAULT 0, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
            $s = $conn->prepare('SELECT project_id, step_number FROM project_steps WHERE step_id = ?'); $s->bind_param('i',$step_id); $s->execute(); $sres = $s->get_result(); 
            $sinfo = $sres ? $sres->fetch_assoc() : null; if (!$sinfo) { echo json_encode(['success'=>false,'message'=>'Step not found']); exit; }
            $projectIdFromStep = (int)$sinfo['project_id']; $stepNumber = (int)$sinfo['step_number']; if ($projectIdFromStep !== $project_id) { echo json_encode(['success'=>false,'message'=>'Step does not belong to this project']); 
            exit; }

            if ($set_done && $stepNumber > 1) { $prev = $conn->prepare('SELECT psp.is_done FROM project_step_progress psp JOIN project_steps ps ON ps.step_id = psp.step_id WHERE ps.project_id = ? AND ps.step_number = ?'); 
                $prev->bind_param('ii',$projectIdFromStep,$stepNumber-1); $prev->execute(); $prevR = $prev->get_result(); $prevRow = $prevR ? $prevR->fetch_assoc() : null; 
                if (!$prevRow || !(int)$prevRow['is_done']) { echo json_encode(['success'=>false,'message'=>'Complete previous step first']); exit; } }
            $up = $conn->prepare('SELECT id FROM project_step_progress WHERE project_id = ? AND step_id = ?'); 
            $up->bind_param('ii',$projectIdFromStep,$step_id); $up->execute(); $upR = $up->get_result(); 
            $exists = $upR ? $upR->fetch_assoc() : null; if ($exists) { $u2 = $conn->prepare('UPDATE project_step_progress SET is_done = ? WHERE id = ?'); 
                $u2->bind_param('ii',$set_done,$exists['id']); $u2->execute(); 
            } else 
            { $i2 = $conn->prepare('INSERT INTO project_step_progress (project_id, step_id, is_done) VALUES (?, ?, ?)'); 
                $i2->bind_param('iii',$projectIdFromStep,$step_id,$set_done); $i2->execute(); }
        ...
        case 'add_step':
            ...
            $snum = $conn->prepare('SELECT COALESCE(MAX(step_number),0)+1 as next_step FROM project_steps WHERE project_id = ?'); 
            $snum->bind_param('i',$project_id); $snum->execute(); $snumR = $snum->get_result(); 
            $snumRow = $snumR ? $snumR->fetch_assoc() : null; $next_step = $snumRow ? $snumRow['next_step'] : 1;
            $ins = $conn->prepare('INSERT INTO project_steps (project_id, step_number, title, instructions) VALUES (?, ?, ?, ?)'); 
            $ins->bind_param('iiss',$project_id,$next_step,$title,$instructions); $ins->execute(); $step_id = $conn->insert_id;
            if (!empty($_FILES['photos']['name'][0])) {
                $upload_dir = 'assets/uploads/';
                foreach ($_FILES['photos']['tmp_name'] as $k=>$tmp) { $fn = uniqid().'_'.basename($_FILES['photos']['name'][$k]); 
                    $t = $upload_dir.$fn; if (move_uploaded_file($tmp,$t)) { $insP = $conn->prepare('INSERT INTO step_photos (step_id, photo_path) VALUES (?,?)'); 
                        $insP->bind_param('is',$step_id,$fn); $insP->execute(); } }
            }
            $conn->commit(); echo json_encode(['success'=>true,'step_id'=>$step_id]); 
            exit;

        case 'update_project_status':
            $status = isset($_POST['status']) ? $_POST['status'] : null; if (!$status) { echo json_encode(['success'=>false,'message'=>'Missing status']); 
                exit; 
            }
            $allowed = ['collecting','in_progress','completed']; 
            if (!in_array($status,$allowed)) { echo json_encode(['success'=>false,'message'=>'Invalid status']); 
                exit; 
            }
            $up = $conn->prepare("UPDATE projects SET status = ?, completion_date = CASE WHEN ? = 'completed' THEN NOW() ELSE NULL END, updated_at = NOW() WHERE project_id = ?"); 
            $up->bind_param('ssi',$status,$status,$project_id); $up->execute(); echo json_encode(['success'=>true]); 
            exit;
        ...
        case 'publish_shared_project':
            ...
            $pstmt->bind_param('i',$project_id); $pstmt->execute(); $pdr = $pstmt->get_result(); 
            $pdata = $pdr ? $pdr->fetch_assoc() : null; if (!$pdata) { throw new Exception('Project not found'); }
            if (!isset($pdata['status']) || $pdata['status'] !== 'completed') { throw new Exception('Only completed projects can be shared'); }
            $ins = $conn->prepare("INSERT INTO shared_projects (project_id, user_id, title, description, cover_photo, tags, privacy, created_at) VALUES (?, ?, ?, ?, ?, ?, 'public', NOW())"); 
            $ins->bind_param('iissss',$project_id,$_SESSION['user_id'],$pdata['project_name'],$pdata['description'],$pdata['cover_photo'],$pdata['tags']); 
            ...
            $mstmt->bind_param('i',$project_id); $mstmt->execute(); $mr = $mstmt->get_result(); while ($m = $mr->fetch_assoc()) 
            { 
                $extra = json_encode(['unit'=>$m['unit'],'is_found'=>(int)$m['is_found']]); $c = $conn->prepare('INSERT INTO shared_materials (shared_id, name, quantity, extra) VALUES (?, ?, ?, ?)'); 
                $c->bind_param('isis',$shared_id,$m['material_name'],$m['quantity'],$extra); $c->execute(); 
            }
            ...
            $sstmt->bind_param('i',$project_id); $sstmt->execute(); $sr = $sstmt->get_result(); while ($srow = $sr->fetch_assoc()) { $insStep = $conn->prepare('INSERT INTO shared_steps (shared_id, step_number, title, instructions, is_done) VALUES (?, ?, ?, ?, ?)'); 
                $insStep->bind_param('iissi',$shared_id,$srow['step_number'],$srow['title'],$srow['instructions'],$srow['is_done']); 
                $insStep->execute(); $new_step_id = $conn->insert_id; $ph = $conn->prepare('SELECT photo_path FROM step_photos WHERE step_id = ?'); 
                $ph->bind_param('i',$srow['step_id']); $ph->execute(); $phr = $ph->get_result(); while ($prow = $phr->fetch_assoc()) 
                { 
                    $insP = $conn->prepare('INSERT INTO shared_step_photos (step_id, path) VALUES (?, ?)'); 
                    $insP->bind_param('is',$new_step_id,$prow['photo_path']); $insP->execute(); } }
            $a = $conn->prepare('INSERT INTO shared_activities (shared_id, user_id, activity_type, data, created_at) VALUES (?, ?, ?, ?, NOW())'); 
            $at = 'publish'; $a->bind_param('iiss',$shared_id,$_SESSION['user_id'],$at,json_encode(['project_id'=>$project_id])); 
            $a->execute(); $conn->commit();
        ...
        case 'shared_activity':
            $shared_id = filter_input(INPUT_POST,'shared_id',FILTER_VALIDATE_INT); $atype = isset($_POST['activity_type']) ? $_POST['activity_type'] : null; 
            if (!$shared_id || !$atype) { echo json_encode(['success'=>false,'message'=>'Missing parameters']); 
                exit; 
            }
            if ($atype === 'like') { $likeStmt = $conn->prepare('SELECT id FROM shared_likes WHERE shared_id = ? AND user_id = ?'); 
                $likeStmt->bind_param('ii',$shared_id,$_SESSION['user_id']); $likeStmt->execute(); $lr = $likeStmt->get_result(); 
                $exists = $lr ? $lr->fetch_assoc() : null; if ($exists) { $d = $conn->prepare('DELETE FROM shared_likes WHERE id = ?'); 
                    $d->bind_param('i',$exists['id']); $d->execute(); $liked = false; } else { $i = $conn->prepare('INSERT INTO shared_likes (shared_id, user_id) VALUES (?, ?)'); 
                        $i->bind_param('ii',$shared_id,$_SESSION['user_id']); $i->execute(); $liked = true; } $insAct = $conn->prepare('INSERT INTO shared_activities (shared_id, user_id, activity_type, data) VALUES (?, ?, ?, ?)'); 
                        $insAct->bind_param('iiss',$shared_id,$_SESSION['user_id'],$atype,json_encode(['liked'=>$liked])); $insAct->execute(); echo json_encode(['success'=>true,'liked'=>$liked]); 
                        exit; 
            }

        case 'unpublish_shared_project':
            $shared_id = filter_input(INPUT_POST,'shared_id',FILTER_VALIDATE_INT); if (!$shared_id) { echo json_encode(['success'=>false,'message'=>'Missing shared id']); 
                exit; 
            }
            $s = $conn->prepare('SELECT user_id FROM shared_projects WHERE shared_id = ?'); $s->bind_param('i',$shared_id); $s->execute(); 
            $sr = $s->get_result(); $owner = $sr ? $sr->fetch_assoc() : null; if (!$owner || $owner['user_id'] != $_SESSION['user_id']) { echo json_encode(['success'=>false,'message'=>'Unauthorized']); 
                exit; 
            } 
            $d = $conn->prepare('DELETE FROM shared_projects WHERE shared_id = ?'); $d->bind_param('i',$shared_id); $d->execute(); echo json_encode(['success'=>true]); 
            exit;
        ...
        case 'generate_share_link':
            ...
            $share_stmt->bind_param('is',$project_id,$share_token); $share_stmt->execute(); 
            $share_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . '/shared_project.php?token='.$share_token; echo json_encode(['success'=>true,'share_url'=>$share_url]); exit;

        case 'update_title':
            $title = _get_str('title'); if (empty($title)) { echo json_encode(['success'=>false,'message'=>'Title required']); 
                exit; 
            } 
            $u = $conn->prepare('UPDATE projects SET project_name = ? WHERE project_id = ?'); 
            $u->bind_param('si',$title,$project_id); $u->execute(); echo json_encode(['success'=>true]); exit;

        case 'update_description':
            $desc = _get_str('description'); if (empty($desc)) { echo json_encode(['success'=>false,'message'=>'Description required']); 
                exit; 
            } 
            $u = $conn->prepare('UPDATE projects SET description = ? WHERE project_id = ?'); 
            $u->bind_param('si',$desc,$project_id); $u->execute(); echo json_encode(['success'=>true]); 
            exit;

        case 'delete_material':
            $material_id = filter_input(INPUT_POST,'material_id',FILTER_VALIDATE_INT); 
            if (!$material_id) { echo json_encode(['success'=>false,'message'=>'Invalid material id']); 
                exit; 
            } 
            $d = $conn->prepare('DELETE FROM project_materials WHERE material_id = ? AND project_id = ?'); 
            $d->bind_param('ii',$material_id,$project_id); $d->execute(); echo json_encode(['success'=>true,'material_id'=>$material_id]); 
            exit;
        ...
        if (isset($_POST['project_id'])) {
            ...
            $verify_stmt = $conn->prepare("SELECT user_id FROM projects WHERE project_id = ?");
            $verify_stmt->bind_param("i", $project_id);
            $verify_stmt->execute();
            $verify_result = $verify_stmt->get_result();
            $project = $verify_result->fetch_assoc();
            
            if ($project && $project['user_id'] == $_SESSION['user_id']) {
                // Delete project materials first (due to foreign key constraints)
                $delete_materials = $conn->prepare("DELETE FROM project_materials WHERE project_id = ?");
                $delete_materials->bind_param("i", $project_id);
                $delete_materials->execute();
                
                // Delete project steps if they exist
                $delete_steps = $conn->prepare("DELETE FROM project_steps WHERE project_id = ?");
                $delete_steps->bind_param("i", $project_id);
                $delete_steps->execute();
                
                // Delete the project
                $delete_project = $conn->prepare("DELETE FROM projects WHERE project_id = ?");
